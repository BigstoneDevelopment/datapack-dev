...
data modify storage bs:data load.modules[{module:"bs.random",version:"3.2.0"}].enabled set value 1b
...
execute unless score #bs.random.major_versions load.status matches 2.. \
  if score $bs.random.major load.status matches 3 \
  if score $bs.random.minor load.status matches 2 \

...
data modify storage bs:data load.modules append value {module:"bs.math",version:"3.2.0",enabled:0b}
...
execute if score $bs.math.major load.status matches 3 if score $bs.math.minor load.status matches 2 unless score $bs.math.patch load.status matches ..0 run return 0
scoreboard players set $bs.math.major load.status 3
scoreboard players set $bs.math.minor load.status 2
scoreboard players set $bs.math.patch load.status 0

...
execute if entity @s[tag=bigstone_sandbox.admin] run data modify storage bigstone_sandbox:data temp.body[0].contents[{"hover_event":{"value":{"translate":"bigstone_sandbox.dialog_menu.import_from_item_data"}}}].click_event.command set value "trigger bigstone_sandbox_menu set -7"

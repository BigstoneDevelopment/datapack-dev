...
tellraw @a ["approximate structure data size: ",{score:{name:"kB",objective:"bigstone_sandbox.commands"}},".",{score:{name:"bytes",objective:"bigstone_sandbox.commands"}}," kB"]
execute if score kB bigstone_sandbox.commands matches 1800.. run tellraw @a [{color:"red",text:"WARNING: structure is most likely too large to export! exporting anyway may result in creating an item with too much data for minecraft to handle. continue at own risk."}]
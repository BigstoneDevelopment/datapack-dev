...
# the player specific data is in: storage xolibs:playerdata storage
...
data modify storage xolibs:playerdata temp.test.UUID set from storage xolibs:playerdata storage.UUID
execute unless function xolibs:sub/playerdata/test run return fail

# saves the currently loaded data to permenant storage
data modify storage xolibs:playerdata temp.save.UUID0 set from storage xolibs:playerdata storage.UUID[0]
data modify storage xolibs:playerdata temp.save.UUID1 set from storage xolibs:playerdata storage.UUID[1]
data modify storage xolibs:playerdata temp.save.UUID2 set from storage xolibs:playerdata storage.UUID[2]
data modify storage xolibs:playerdata temp.save.UUID3 set from storage xolibs:playerdata storage.UUID[3]
...
data modify storage xolibs:playerdata storage.UUID set from storage xolibs:playerdata temp.test.UUID
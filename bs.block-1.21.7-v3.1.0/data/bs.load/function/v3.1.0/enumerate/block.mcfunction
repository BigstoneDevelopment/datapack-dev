...
data modify storage bs:data load.modules append value {module:"bs.block",version:"3.1.0",enabled:0b}
...
execute if score $bs.block.major load.status matches 3 if score $bs.block.minor load.status matches 1 unless score $bs.block.patch load.status matches ..0 run return 0
